                            ...
                            item {
                                AnimatedVisibility(
                                    ...
                                    Column(modifier = Modifier.padding(start = 16.dp, top = 8.dp, bottom = 8.dp)) {
                                        sets.forEach { set ->
                                            LogSetItem(set = set, weightUnit = weightUnit)
                                        }
                                    }
                                }
                            }
...
@Composable
fun LogSetItem(set: CompletedSet, weightUnit: WeightUnit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(vertical = 4.dp)
        ...
        Column(
            ...
            Row(
                ...
                if (set.weightAdded != null) {
                    ...
                    Text("Weight: ${String.format("%.1f", displayWeight)} ${weightUnit.name.lowercase()}", style = MaterialTheme.typography.bodyLarge)
                }
            }
